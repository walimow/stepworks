...
namespace  stepworks{
    ...
    template < typename Corpus, typename Elm=unsigned char>
    struct itx{
       ...
       template <  template <typename...> typename stream_t >
        ...
        template <  template <typename...> typename stream_t >        
        static  constexpr auto read_corpus_v( stream_t<Elm>&& c ){
            auto ret =try_read_corpus<stream_t>(fw<Elm, stream_t, std::optional>{ std::move(c) }  );
            return ret;
        }
    };
    
    ///convinience for write  (read is not possible so:  you have to define the type of return)
    template < typename Corpus, template<typename... >typename Form = std::list,  typename Elm=unsigned char>
    auto itx_write(const Corpus& c, Form<Elm>&& fe=Form<Elm>{}){
        return itx< Corpus, Elm>::write_corpus_forward_ret(std::move(fe), c);
    }
    
    template <  template<typename... >typename Form ,typename Corpus, 
    typename...Xs,  
    typename Elm=unsigned char>
    auto itx_write2(const Corpus& c, Form< Elm , Xs... >&& fe=Form< Elm /*, Xs...*/ >{}){
        return itx< Corpus, Elm>::write_corpus_forward_ret(std::move(fe), c);
    }
}
